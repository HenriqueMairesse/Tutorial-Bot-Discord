...
const handler: EventHandler<'clientReady'> = async (client) => {
    ...
    setInterval(async () => {
      ...
      const sorteios = await sorteioSchema.find({dataTermino: {$lte: Date.now()}, terminou: false});
      if (!sorteios || sorteios.length == 0) return;
      sorteios.forEach(async (sorteio) => {
        const guild = await client.guilds.fetch(sorteio.guild);
        ...
        canal.send(`Parabéns ao(s) ganhador(es) de ${sorteio.premio}! Ganhadores: ${giveawayRoll(sorteio.participantes, sorteio.ganhadores)}`);
